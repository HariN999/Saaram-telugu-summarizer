...
\title{Morphology-Aware Telugu News Summarization:
Comparing Extractive and Abstractive Approaches
under Resource Constraints}

\titlerunning{Morphology-Aware Telugu News Summarization}
...
%% ============================================================
\begin{abstract}
Telugu is a morphologically rich, agglutinative language that poses challenges for tokenization and evaluation in standard NLP pipelines. We present
\textit{Saaram} ({\telugufont సారం}, ``essence''), a deployed Telugu news summarization service with optional speech synthesis, and conduct a comparative study of five configurations evaluated on the complete 1,302-sample XL-Sum Telugu test split. Our morphology-aware TF-IDF baseline replaces word-level tokenization with character n-gram analysis (\texttt{analyzer='char\_wb'}, $n \in [2,4]$) and lead-bias position weighting, achieving a 26.6\% improvement in ROUGE-2 over the word-level baseline. Evaluated abstractive models show that the 8-bit LoRA fine-tuned mT5 model achieves the highest lexical overlap (ROUGE-2: 0.0496) but yields near-identical semantic quality, measured by BERTScore (0.7272), compared to mT5 base (0.7273), indicating that in-distribution fine-tuning provides negligible semantic gains. Finally, a resource-aware runtime router dynamically selects between TF-IDF and mT5 base, retaining 99.56\% of mT5 semantic quality (BERTScore) while mitigating memory exhaustion risk. End-to-end latency profiling further provides deployment-relevant cost--quality trade-offs under resource constraints.
\end{abstract}
...
digital Telugu news creates a clear societal need: readers face
an overwhelming volume of content with few publicly accessible automated summarization tools exist for Telugu at scale.

Two structural challenges compound this problem. First, Telugu
is \emph{agglutinative}: a single root word generates dozens of
inflected surface forms through affixation. Standard NLP
pipelines that assume word-level tokenization — designed for
morphologically simple languages like English — treat these
inflected forms as unrelated vocabulary items, severely
underestimating term importance and fragment evaluation
...
\textit{{\telugufont రాష్ట్రానికి}} (to the state) — three tokens that
word-level TF-IDF treats as entirely unrelated.
Second, many standard ROUGE implementations strip non-ASCII characters by default, making Telugu ROUGE scores
undefined without explicit adaptation.

Prior Telugu NLP work has addressed these challenges in
isolation: extractive baselines without morphological
awareness~\cite{anandbabu2023extractive}, or neural
abstractive models without deployment
analysis~\cite{srinivasa2019neural}. To our knowledge,
no prior Telugu summarization study combines
morphology-aware extractive improvement, large-scale
neural comparison, and production deployment tradeoff
analysis in a single evaluation framework.

This paper presents \textit{Saaram} ({\telugufont సారం}, ``essence''), a
deployed Telugu news summarization service with speech synthesis, and reports
five contributions:

\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
  \item A \textbf{morphology-aware TF-IDF baseline} (TF-IDF~V2) utilizing character n-grams and lead-bias weighting, achieving a 26.6\% ROUGE-2 improvement over word-level TF-IDF~V1.
  \item A \textbf{large-scale comparative benchmark} comparing five methods on the complete 1,302-sample XL-Sum Telugu test split with full ROUGE, BERTScore, and latency profiling.
  \item A \textbf{LoRA fine-tuning ablation study} showing that 8-bit quantized LoRA adaptation yields marginal gains (+0.0032 ROUGE-L) without semantic improvement (BERTScore: 0.7272 vs. 0.7273) over the pre-trained base.
  \item A \textbf{resource-aware inference strategy} routing requests at runtime to retaining 99.56\% of mT5 semantic quality (BERTScore) with a 5.38\% routing rate (70 of 1,302 samples) to TF-IDF~V2, serving as a deployment fallback.
  \item A \textbf{resource-constrained deployment analysis} of the system on constrained free-tier CPU cloud infrastructure, characterizing memory-aware lazy loading, fallbacks, and TTS latency.
\end{itemize}

The complete system is publicly deployed: frontend on Vercel,
backend on Hugging Face Spaces (Docker, CPU-only), and source
code on GitHub.
...
All ROUGE values are F1 scores.
BERTScore F1 computed using \texttt{bert-base-multilingual-cased} with rescaling.
...
\begin{table}[!htbp]
\centering
\caption{Inference Latency Profile --- CPU-only deployment
(HuggingFace Spaces, 1,302 samples).}
\label{tab:latency}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.9cm}cc}
\toprule
\textbf{Method} & \textbf{Mean (s)} & \textbf{P95 (s)} \\
\midrule
TF-IDF V1        & 0.001 & 0.001 \\
TF-IDF V2        & 0.005 & 0.008 \\
mT5 Base         & 2.583 & 3.739$^*$ \\
mT5 Fine-tuned$^\dagger$ & 2.482 & --- \\
Auto Router      & 3.130 & 5.504 \\
\midrule
\multicolumn{3}{p{8.2cm}}{\footnotesize
$^*$ mT5 Base P99 = 9.689\,s. 
$^\dagger$ Fine-tuned model uses 8-bit LoRA; latency reduced versus prior full-parameter checkpoint (3.372\,s; Sect.~\ref{sec:mt5}).} \\
\multicolumn{3}{p{8.2cm}}{\footnotesize
P95/P99 omitted for sub-ms TF-IDF. Fine-tuned P95/P99 not measured.
Router P99 omitted due to cold-start variance.} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{(F3) Deployment latency remains comparable.}
